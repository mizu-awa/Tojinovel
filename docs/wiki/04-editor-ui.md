...
| 保存 | プロジェクトフォルダ内の `data/gamedata.json` に保存します |
| ▶（デバッグプレイ） | 未保存なら保存してからデバッグプレイ画面に遷移します |
| ⬇（書き出し） | クリックするとメニューが開きます（下記参照） |
| 📂（プロジェクト選択） | 保存してからプロジェクト選択画面に戻ります |

### 書き出しメニュー

| 項目 | 説明 | 対象 |
|---|---|---|
| ゲーム出力 (HTML) | プレイヤー用ファイル（index.html / assets/）をプロジェクトフォルダに出力します | Wails版・ブラウザ版共通 |
| プロジェクトZIP | プロジェクト全体をZIPファイルとしてダウンロードします | ブラウザ版のみ |
...
プロジェクトのファイルをアプリ内で管理できます。

- **ファイルツリー表示**: `data/`, `system/` などのフォルダ・ファイルを一覧表示
- **パスコピー**: ファイルをクリックすると相対パス（例: `./data/images/bg.png`）をクリップボードにコピー
- **右クリックメニュー**: ファイルの削除・リネーム・パスコピー
- **ファイル追加**: 「追加」ボタン → ファイル選択ダイアログ → プロジェクトにコピー
  - Wails版: OSのファイル選択ダイアログ
  - ブラウザ版: ブラウザのファイル選択ダイアログ（画像・音声ファイル対応）
- **リフレッシュ**: ツリーを最新状態に更新
